...
if __name__ == "__main__":
    ...
    rm_if_exist(f'{target_folder}/nc_a')
    os.makedirs(f'{target_folder}/nc_a', exist_ok=True)
    for label in range(param["num_classes"]):
        trigger, mask = train(net, label, lodaer, param)
        norm_list.append(mask.sum().item())
        torchvision.utils.save_image(mask, f'{target_folder}/nc_a/mask_{label}.png', normalize=True)
        torchvision.utils.save_image(trigger * mask, f'{target_folder}/nc_a/trigger_{label}.png', normalize=True)
    print(norm_list)
    anomaly_index = outlier_detection(norm_list)

    with open(f'{target_folder}/nc_a/res.txt', 'w') as file:
